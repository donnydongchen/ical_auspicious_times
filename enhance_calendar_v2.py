...
def enhance_trunk_branch(lookup):
    ...
    # Split by VEVENT while preserving everything else
    parts = content.split('BEGIN:VEVENT')
    header = parts[0]  # Everything before first VEVENT
    
    output_lines = [header]  # Start with header
    event_count = 0
    
    # Process each event
    for event_part in parts[1:]:
        event_count += 1
        progress_bar(event_count, total_events)
        
        # Reconstruct event text
        event_text = 'BEGIN:VEVENT' + event_part
    ...
    # Write output file
    print(f"\n\n[3/3] Writing enhanced file to {OUTPUT_FILE}...\n")
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        f.write(''.join(output_lines))
    
    print(f"✓ Enhanced file written to {OUTPUT_FILE}\n")
...
def generate_report():
    ...
    if stats['samples']:
        ...
        for i, sample in enumerate(stats['samples'], 1):
            ...
            before_preview = sample['before'][:72] + ('...' if len(sample['before']) > 72 else '')
            after_preview = sample['after'][:72] + ('...' if len(sample['after']) > 72 else '')
